#if FW35
// BASEDON: https://github.com/dotnet/corert/blob/4376c55dd018d8b7b9fed82449728711231ba266/src/System.Private.CoreLib/src/System/Lazy.cs
...
namespace System
{
	...
	public class Lazy<T>
	{
		...
		private static Func<T> s_ALREADY_INVOKED_SENTINEL = () => default(T);
		...
		private Boxed CreateValue()
		{
			...
			else
			{
				...
				{
					Exception ex = new MissingMethodException("No parameterless ctor for T");
					if (mode != LazyThreadSafetyMode.PublicationOnly) // don't cache the exception for PublicationOnly mode
						_boxed = new LazyInternalExceptionHolder(ex);
					throw ex;
				}
