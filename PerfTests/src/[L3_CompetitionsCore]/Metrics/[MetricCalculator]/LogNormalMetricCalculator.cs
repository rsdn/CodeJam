...
namespace CodeJam.PerfTests.Metrics
{
	...
	public sealed class LogNormalMetricCalculator : IMetricCalculator
	{
		...
		#region Helpers
		private static bool TrySetLogValues(ref double[] values)
		{
		...
		private static bool TrySetLogValues(ref double[] values, ref double[] baselineValues)
		{
		...
		{
			if (!TrySetLogValues(ref values, ref baselineValues))
				return null;
		...
		public double? TryGetVariance(double[] values)
		{
			...
			// σValues
			return new Statistics(values).StandardDeviation;
		}
