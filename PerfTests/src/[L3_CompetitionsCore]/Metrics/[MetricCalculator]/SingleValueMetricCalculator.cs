...
namespace CodeJam.PerfTests.Metrics
{
	...
	public sealed class SingleValueMetricCalculator : IMetricCalculator
	{
		...
		public double? TryGetRelativeVariance(double[] values, double[] baselineValues) => null; // TODO: !!! variance?
		...
		public MetricRange TryGetActualValues(double[] values)
		{
			...
			return result == null ? MetricRange.Empty : MetricRange.Create(result, result);
		}
		...
		[Pure]
		public MetricRange TryGetLimitValues(double[] values) => TryGetActualValues(values);
		...
		/// <returns>Limit values range for the set of values (or empty range if none).</returns>
		[Pure]
		public MetricRange TryGetRelativeLimitValues(double[] values, double[] baselineValues) => TryGetActualValues(values);
	}
}