	...
	public static partial class BenchmarkActionFactory
	{
		...
		internal abstract class BenchmarkActionBase : BenchmarkAction
		{
				...
				int unrollFactor)
			{
				...
				var storeResultField = GetStoreResultField(
					instanceType, storeResultFieldName, callbackInvokeMethod.ReturnType);

				// void InvokeMultipleEmitted(long x) // instance method associated with instanceType
				var m = new DynamicMethod(
					"InvokeMultipleEmitted", typeof(void), new[] { instanceType, typeof(long) }, instanceType)
				{
					InitLocals = true
				};
			...
			private static FieldInfo GetCallbackField(Type instanceType, string callbackFieldName)
			{
				...
				if (callbackFieldType != typeof(Action) &&
					(!callbackFieldType.GetTypeInfo().IsGenericType
						|| callbackFieldType.GetGenericTypeDefinition() != typeof(Func<>)))
